...
const SelectList = ({ children }: SelectProps) => {
  return <div className={style.select}>{children}</div>;
};
...
export const Select = ({
  ...
  const getLabelByValue = (value: any) =>
    options.find((m) => m.value === value)?.label || "";
  ...
  const filteredList = options.filter((f) =>
    removeAccent(`${f.label}`).includes(removeAccent(search)),
  );
