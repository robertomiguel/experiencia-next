...
export const CropImagen = ({ onChange }: ImagenProps) => {
  ...
  const onCropComplete = useCallback(
    async (_: any, croppedImg: any) => {
      const croppedImage = await getCroppedImg(imagen, croppedImg, rotation);
  ...
  return (
    <div className="flex flex-col w-full justify-around">
      ...
      <div className="flex flex-row w-full justify-around">
        {imagen && (
          <>
            <div className="relative bg-white w-[500px] h-[500px] inline-block border-2 border-blue-300">
              <Cropper
                image={imagen}
                crop={crop}
                rotation={rotation}
